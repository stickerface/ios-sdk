...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (!((UIScrollView *)view.superview).refreshControl.isRefreshing) {
            [UIView performWithoutAnimation:^{
                view.frame = CGRectMake(0.0, -((UIScrollView *)view.superview).adjustedContentInset.top, view.superview.frame.size.width, view.superview.frame.size.height);
            }];
        }
